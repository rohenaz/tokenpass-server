...
class State {
	...
	findOne(o: Partial<StateRecord>): Promise<StateRecord | null> {
		return new Promise((resolve) => {
			this.db.findOne(o, (_err, state: StateRecord | null) => {
				if (state) {
	...
	find(o: Partial<StateRecord>): Promise<StateRecord[]> {
		return new Promise((resolve) => {
			this.db.find(o, (_err, states: StateRecord[]) => {
				resolve(states);
			});
		});
	}
	...
	delete(o: Partial<StateRecord>): Promise<number> {
		return new Promise((resolve) => {
			this.db.remove(o, (_err, numRemoved: number) => {
				resolve(numRemoved);
			});
		});
	}
	...
	count(o: Partial<StateRecord>): Promise<number> {
		return new Promise((resolve) => {
			this.db.count(o, (_err, count: number) => {
				resolve(count);
			});
		});
	}
	...
	insert(state: StateRecord): Promise<StateRecord> {
		return new Promise((resolve) => {
			this.db.insert(state, (_err, _doc) => {
				this.setState(state);
				resolve(state);
			});
		});
	...
	update(state: StateRecord): Promise<StateRecord> {
		return new Promise((resolve) => {
			this.db.update(
				...
				(err, _numReplaced, doc: StateRecord) => {
	...
	all(): Promise<StateRecord[]> {
		return new Promise((resolve) => {
			this.db.find({}, (_err, states: StateRecord[]) => {
				resolve(states);
			});
		});
	}
